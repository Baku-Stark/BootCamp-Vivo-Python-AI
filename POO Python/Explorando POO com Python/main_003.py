...
class Plano:
    ...
    def deduzir_saldo(self, duracao):
        return self.verificar_saldo() - self.custo_chamada(duracao)
...
class UsuarioTelefone:
    def __init__(self, nome, numero, plano):
        self.nome = nome
        self.numero = numero
        self.plano = plano # classe 'Plano'

    # TODO: Crie um método fazer_chamada para permitir que um usuário faça uma chamada telefônica:
    def fazer_chamada(self, destinatario, duracao):
        mensagem, valor = self.valor_chamada(duracao)
        
        if valor:
            return f"Chamada para {destinatario} realizada com sucesso. Saldo: ${valor:.2f}"

        else:
            return mensagem
        
    # TODO: Calcule o custo da chamada usando o método 'custo_chamada' do objeto 'plano':
    def valor_chamada(self, duracao):
        mensagem = any
        valor = 0

        # TODO: Verifique se o saldo do plano é suficiente para a chamada.
        if self.plano.verificar_saldo() > self.plano.custo_chamada(duracao):
            # TODO: Se o saldo for suficiente, deduz o custo da chamada do saldo do plano.
            valor = self.plano.deduzir_saldo(duracao)

        else:
            mensagem = "Saldo insuficiente para fazer a chamada."
